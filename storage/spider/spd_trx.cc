...
/**
  Set link indices for load balancing read connections.

  Assuming `spider->share->link_count` is the number of active servers
  to use, this function tries to find these number of servers whose
  link status is not `SPIDER_LINK_STATUS_NG`. To that end, for each
  `link_idx` less than the link count, look for a server with the same
  index modulus link count with the desired link status. If one cannot
  be found, use the `link_idx`th server.
*/
void spider_trx_set_link_idx_for_all(
  ...
  for (uint link_idx = 0; link_idx < link_count; link_idx++)
  {
    ...
    DBUG_ASSERT(link_idx == conn_link_idx[link_idx]);
